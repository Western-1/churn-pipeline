...
class TestPredictionEndpoint:
    ...
    def test_predict_returns_probability(self, sample_inference_data):
        ...
        if response.status_code == 200:
            ...
            has_probability = any(
                key in data for key in ["probability", "confidence", "score", "proba"]
            )

            if has_probability:
                prob_key = next(
                    key for key in ["probability", "confidence", "score", "proba"] if key in data
                )
    ...
    def test_predict_response_time(self, sample_inference_data):
        ...
        client.post("/predict", json=sample_inference_data)
        end_time = time.time()
...
class TestBatchPredictionEndpoint:
    ...
    def test_batch_predict_endpoint(self, sample_inference_data):
        ...
        # Try batch endpoint
        batch_data = {"instances": [sample_inference_data, sample_inference_data]}
...
class TestCORS:
    ...
    def test_cors_headers_present(self, sample_inference_data):
        ...
        _ = client.post("/predict", json=sample_inference_data)
